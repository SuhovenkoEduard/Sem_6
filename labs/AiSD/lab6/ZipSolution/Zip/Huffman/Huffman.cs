...
namespace ConsoleApp1.Huffman
{
    public static class Huffman
    {
        ...
        {
            var parts = Parts.NumberOfParts(inputFilePath);
            for (var i = 0; i <= parts; i++)
                File.Delete(Parts.GetPartFilePath(archDirPath, archBaseFileName, i));

            Stream ifstreamIn = new FileStream(inputFilePath, FileMode.Open, FileAccess.Read);
            Stream ofstreamIn = new FileStream(Parts.GetFilePath(archDirPath, archBaseFileName), FileMode.Create,
            ...
            var res = new FileInfo(Parts.GetFilePath(archDirPath, archBaseFileName)).Length;
            Console.WriteLine("koef " + koef / Convert.ToDouble(res));
            ofstreamIn.Write(bufferIn, 0, bufferIn.Length);
            ofstreamIn.Close();
            Parts.ToParts(Parts.GetFilePath(archDirPath, archBaseFileName), archDirPath, archBaseFileName, sizeArch);
            File.Delete(Parts.GetFilePath(archDirPath, archBaseFileName));
            ifstreamIn.Close();
        }
        ...
        private static void Huff(Stream sin, Stream sout, UpdateCrc callback)
        {
            ...
            ret = t.GetCode(256);
            while (ret.Count > 0) bw.WriteBit(ret.Pop());
        }


        public static void Decompress(string archDirPath, string archBaseFileName, string outputFilePath)
        {
            var bytes = Parts.FromParts(archDirPath, archBaseFileName);
            File.WriteAllBytes(Parts.GetFilePath(archDirPath, archBaseFileName), bytes);
            Stream ifstream = new FileStream(Parts.GetFilePath(archDirPath, archBaseFileName), FileMode.Open,
                FileAccess.Read);
            ...
            while (myDecompressor.IsAlive) Thread.Sleep(100);
